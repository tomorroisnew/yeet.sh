...
~/tools/massdns/bin/massdns -r ~/tools/massdns/lists/resolvers.txt -t A -o S altdns2.txt -w lmao2.txt

sed 's/A.*//' lmao2.txt | sed 's/CN.*//' | sed 's/\..$//' | sort -u > massdns2.txt

cat massdns2.txt passiveoutput.txt| sort -u | grep $1 | httpx -title -status-code -o final.txt -location -content-length -no-color
